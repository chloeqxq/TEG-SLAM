...
BUNDLE_ROOT = REPO_ROOT / "output" / "wandering_turn_bundle_v4"
VERSIONS = {
    "v3": REPO_ROOT
    / "output"
    / "Wild_SLAM_iPhone_temporal_proposal_v3_turn"
    / "iphone_wandering_temporal_proposal_v3_turn",
    "persongate": REPO_ROOT
    / "output"
    / "Wild_SLAM_iPhone_temporal_persongate_turn"
    / "iphone_wandering_temporal_persongate_turn",
    "v4": REPO_ROOT
    / "output"
    ...
    "v4plus": REPO_ROOT
    / "output"
    / "Wild_SLAM_iPhone_temporal_proposal_v4plus_turn"
    / "iphone_wandering_temporal_proposal_v4plus_turn",
}
SELECTED_FRAMES = [
...
def write_readme():
    ...
    readme.write_text(
        "\n".join(
            [
                "# Turn Experiment Bundle",
                "",
                "Bundled outputs for the turn-focused iPhone wandering experiments.",
                "",
                "Included versions:",
                "- `v3`",
                "- `persongate`",
                ...
                "- `rendered_rgb_after_refine.mp4`",
                "- `output.gif`",
                "- `selected_frames/` with the turn-region keyframes",
                "",
                "Included shared compare views:",
                "- `compare_frames/` with horizontal grids",
            ]
        )
        + "\n",
        encoding="utf-8",
...
if __name__ == "__main__":
    ...
    write_readme()
    print(f"Bundled outputs into: {BUNDLE_ROOT}")
