...
def _load_metric3d_from_hub(depth_model: str) -> torch.nn.Module:
    """Avoid shadowing the real mmcv package with a local empty directory."""
    repo_root = Path(__file__).resolve().parents[3]
    shadow_mmcv_dir = repo_root / "mmcv"
    removed_paths = []
    if shadow_mmcv_dir.is_dir():
        repo_root_str = str(repo_root)
        removed_paths = [path for path in sys.path if path == repo_root_str]
        if removed_paths:
            sys.path = [path for path in sys.path if path != repo_root_str]

    try:
        return torch.hub.load("yvanyin/metric3d", depth_model, pretrain=True)
    finally:
        if removed_paths:
            sys.path = removed_paths + sys.path
...
def get_metric_depth_estimator(cfg: Dict) -> torch.nn.Module:
    ...
    if "metric3d_vit" in depth_model:
        ...
        model = _load_metric3d_from_hub(depth_model)
