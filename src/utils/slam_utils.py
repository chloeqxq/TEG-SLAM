...
def get_loss_mapping_uncertainty(
    ...
    temporal_prior: Optional[Tensor] = None,
    temporal_static_evidence: Optional[Tensor] = None,
    ...
    raw_uncertainty = uncertainty_network(features)
    uncertainty = raw_uncertainty
    uncertainty_already_adjusted = False

    if (
        temporal_prior is not None
        and temporal_static_evidence is not None
        and temporal_params is not None
        and temporal_params.get("activate", False)
        and temporal_params.get("mapping_use_temporal_posterior", True)
    ):
        data_rate = 1 + map_utils.compute_bias_factor(train_frac, 0.8)
        adjusted_uncertainty = temporal_utils.apply_uncertainty_data_rate(
            raw_uncertainty, data_rate
        )
        uncertainty, _ = temporal_utils.fuse_uncertainty_with_prior(
            adjusted_uncertainty,
            temporal_prior,
            temporal_static_evidence,
            off_threshold=temporal_params.get("off_threshold", 0.35),
            release_threshold=temporal_params.get("release_threshold", 0.60),
            min_weight=temporal_params.get("min_weight", 0.05),
            fusion_mode=temporal_params.get("fusion_mode", "heuristic"),
            log_odds_measurement_gain=temporal_params.get(
                "log_odds_measurement_gain", 1.0
            ),
            log_odds_probability_floor=temporal_params.get(
                "log_odds_probability_floor", 0.15
            ),
            log_odds_clip=temporal_params.get("log_odds_clip", 4.0),
        )
        uncertainty_already_adjusted = True
    ...
    uncer_loss, uncer_resized, l1_rgb, l1_depth = map_utils.compute_mapping_loss_components(
        ...
        mask=rgb_pixel_mask,
        uncertainty_already_adjusted=uncertainty_already_adjusted,
    )
